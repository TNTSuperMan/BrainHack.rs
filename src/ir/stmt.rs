...
fn parse_block(ctx: &mut ParserContext, statement: &Statement) -> Result<Vec<IRStmt>> {
    if let Statement::Block(block) = statement {
        block
            .statement_list()
            .iter()
            .map(|s| parse_statement_item(ctx, s))
            .collect()
    } else {
        parse_stmt(ctx, statement).map(|stmt| vec![stmt])
    }
}
...
pub fn parse_stmt(ctx: &mut ParserContext, statement: &Statement) -> Result<IRStmt> {
    match statement {
                ...
                if let Expression::Identifier(id) = call.function() {
                    if id.sym() == ctx.interner.get("out").unwrap() {
                        ...
                        call.args()
                            ...
                            .map(|args| IRStmt::Call { id: id.sym(), args })
                    }
                } else {
                ...
                if let UpdateTarget::Identifier(id) = upd.target() {
                    ...
                    Ok(match upd.op() {
                        UpdateOp::IncrementPost | UpdateOp::IncrementPre => IRStmt::Assign {
                            id: id.sym(),
                            value: IRExpr::Add(id_expr, one_expr),
                        },
                        UpdateOp::DecrementPost | UpdateOp::DecrementPre => IRStmt::Assign {
                            id: id.sym(),
                            value: IRExpr::Sub(id_expr, one_expr),
                        },
                    })
                } else {
        ...
        Statement::Block(_) => Ok(IRStmt::Block {
            body: parse_block(ctx, statement)?,
        }),
        Statement::ForLoop(for_ast) => {
            ...
            if let Some(init) = for_ast.init() {
                body.push(parse_stmt(
                    ...
                    &match init {
                        ...
                        ForLoopInitializer::Lexical(lex) => Statement::Var(VarDeclaration(
                            lex.declaration().variable_list().clone(),
                        )),
            ...
            body.push(IRStmt::While {
                condition: for_ast
                    .condition()
                    .map_or(Ok(IRExpr::Const(1)), |e| parse_expr(ctx, e))?,
                body: loop_body,
            });
