...
pub fn compile_expr(
    ...
    match expr {
        ...
        IRExpr::Mul(l, r) => match (l.as_ref(), r.as_ref()) {
            (IRExpr::Const(nl), IRExpr::Const(nr)) => {
                asm.push(AsmOp::Set(target, nl.wrapping_mul(*nr)))
            }
            (IRExpr::Const(n), exp) | (exp, IRExpr::Const(n)) => {
        ...
        IRExpr::Call { id, args } => {
            ...
            expr!(
                target,
                func.result.as_ref().unwrap_or_else(|| &IRExpr::Const(0))
            );
        ...
        IRExpr::BoolNot(exp) => {
            ...
            asm.push(AsmOp::Loop(
                tmp,
                vec![AsmOp::Set(target, 0), AsmOp::Set(tmp, 0)],
            ));
            ctx.free(tmp)?;
        }
        IRExpr::Boolify(exp) => {
            ...
            asm.push(AsmOp::Loop(
                tmp,
                vec![AsmOp::Set(target, 1), AsmOp::Set(tmp, 0)],
            ));
            ctx.free(tmp)?;
        }
        IRExpr::Gt(expl, expr) => {
            ...
            asm.push(AsmOp::Loop(
                ...
                vec![
                    ...
                    AsmOp::Loop(
                        r,
                        vec![AsmOp::Set(tmp0, 0), AsmOp::Add(tmp1, 1), AsmOp::Add(r, -1)],
                    ),
